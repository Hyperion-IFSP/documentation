...
\begin{figure}[!htbp]
    ...
    \label{utiliza_app}
\end{figure}
Essa pergunta foi escolhida porque é a base para nossa aplicação. Esse dado é bastante relevante, pois demonstra que há uma lacuna tecnológica quando se trata da rotina de controle de medicamentos entre os profissionais. 
...
\end{figure}
A pergunta 9 foi selecionada para avaliar se a aplicação seria vantajosa e útil para o nosso público-alvo. Os resultados obtidos são especialmente relevantes, pois indicam que há interesse dos profissionais em incorporar a tecnologia em suas atividades diárias.

Como apresentado nas figuras 1 e 2, respectivamente, há uma lacuna quando se trata da rotina de controle de medicamentos de seus pacientes, mas ao mesmo tempo, há um interesse em fazer uma conversão para um sistema que atenda melhor essas necessidades. 
...
\subsubsection{Detalhamento do Contêiner}
...
\textit{Infraestrutura:} Hospedada na plataforma Firebase.\\
...
\begin{enumerate}
    \item O usuário acessa a aplicação web hospedada na Firebase.
...
\section{Seção empresa}
Na disciplina de Gestão Industrial (GEI), como atividade avaliatica para composição de nota do segundo bimestre, desenvolvemos a estrutura empresarial da equipe Hyperion, cujo foco é o aplicativo \nomeprojeto. O projeto apresenta a identidade da empresa, análise de mercado, modelo de negócio, estratégias de marketing e outros. A descrição completa da empresa pode ser consultada no \autoref{secao_empresa}
