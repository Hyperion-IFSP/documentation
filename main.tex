...
No âmbito técnico, surgiram dificuldades relacionadas à compatibilidade de versões e bibliotecas, erros de integração entre módulos e limitações impostas por algumas ferramentas utilizadas. Em certos momentos, a falta de pesquisas e dcumentações dificultaram o embasamento do projeto, em outros momentos, a necessidade constante de bugs demandaram um tempo que não estava previsto no cronograma, atrasando outras etapas do desenvolvimento. 
Referente aos aspectos organizacionais, destacaram-se obstáculos no gerenciamento de tempo e na adaptação do cronograma, pricipalmente quando houve necessidade de retrabalho


ocasionados por mudanças de escopo. Além disso, em determinados momentos, a comunicação entre orientador e equipe precisou ser aprimorada para alinhar expectativas e direcionamentos.

Por fim, em relação a fatores externos, houve limitações no acesso a dados reais para testes, especialmente devido a questões de privacidade e confidencialidade. Também ocorreram dificuldades em depender de serviços de terceiros, como APIs externas, que em alguns casos apresentaram instabilidade ou alterações inesperadas.
...
\subsection {Problemas nos testes internos}

\subsection {Problemas nos testes externos}
