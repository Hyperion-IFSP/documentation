...
Durante o desenvolvimento do trabalho, foram identificados diversos problemas que impactaram tanto o andamento do cronograma quanto a implementação de determinadas funcionalidades. 

No âmbito técnico, surgiram dificuldades relacionadas à compatibilidade de versões e bibliotecas, erros de integração entre módulos e limitações impostas por algumas ferramentas utilizadas. Além disso, o banco de dados precisou ser migrado para uma instância diferente, o que trouxe ajustes adicionais e exigiu atenção especial, visto que a instância precisa ser renovada a cada 30 dias e todas as migratons devem ser executadas novamente. 

Outro desafio esteve na camada de segurança, O front-end não conseguiu criptografar as requisições como esperado, resultando em um envio de dados não totalmente seguro. Também não foi possível implementar token de sessão do backend para requisições, apenas o credenciamento;

Por fim, apesar dos esforços de otimização, não foi possível atingir a latência média de 200ms - valor considerado ideal -, embora os resultados obtidos tenham se mostrado satisfatórios para a maioria das operações.

Em certos momentos, a falta de pesquisas e documentações dificultaram o embasamento do projeto; em outros, a necessidade constante de correções de bugs demandou um tempo que não estava previsto no cronograma, atrasando outras etapas do desenvolvimento.

Apesar dessas dificuldades, os problemas enfrentados contribuíram para o amadurecimento do processo de pesquisa e desenvolvimento. Cada obstáculo superado proporcionou aprendizado e tornou possível a adaptação do trabalho, assegurando a continuidade do projeto e a consolidação dos resultados apresentados.
...
\subsubsection{Aspectos positivos}
Apesar das dificuldades, foram observados avanços significativos no sistema. A API retorna erros de forma clara e estruturada, permitindo que sejam exibidos adequadamente no front-end em situações de exceção. O tempo médio de resposta da API mostrou-se inferior ao previsto no início do desenvolvimento, garantindo maior eficiência na comunicação entre módulos. Além disso, a API foi documentada de maneira explícita, dispensando auxílio constante da equipe de desenvolvimento e facilitando a compreensão por terceiros. Por fim, o front-end apresenta uma estrutura robusta e limpa, oferecendo navegação intuitiva entre as páginas e proporcionando melhor usabilidade para os usuários.

\subsubsection{Aspectos negativos}
Durante os testes externos, alguns problemas foram identificados e merecem destaque. O login não é validado corretamente no backend, comprometendo a segurança do sistema. Além disso, não foi possível abrir a foto do paciente no backend, o que limita a visualização de informações relevantes. Outro fator crítico está relacionado ao tempo de inicialização da API, que pode levar alguns segundos até ficar ativa, impedindo o acesso imediato à aplicação. Soma-se a isso a ausência de implementação das claims/roles, permitindo que qualquer usuário realize requisições sem restrições, o que representa um risco de uso indevido. Por fim, a hospedagem do front-end não se mostrou estável, impossibilitando que as versões mais recentes da aplicação cheguem de forma consistente ao usuário final.
...
\section{Conclusão}
O desenvolvimento do \nomeprojeto demonstrou a importância da tecnologia como ferramenta de apoio à saúde e ao cuidado de idosos, especialmente em um contexto onde a demanda por serviços de cuidado está em crescimento. Através da análise das necessidades dos cuidadores e da implementação de funcionalidades especialmente aqueles que dependem de cuidadores para suas atividades diárias. Ao longo do trabalho, foi possível identificar tanto avanços significativos quanto desafios complexos. 

Entre os aspectos positivos, destaca-se a criação de uma API eficiente e bem documentada, que facilita a integração e o uso por terceiros. A estrutura do front-end também se mostrou robusta, proporcionando uma navegação intuitiva e uma boa experiência para os usuários. No entanto, desafios técnicos relacionados à segurança, desempenho e estabilidade da aplicação foram identificados, indicando áreas que necessitam de melhorias contínuas. Mesmo diante de limitações técnicas, esses resultados evidenciam que o projeto alcançou, em grade parte, seus objetivos de promover acessibilidade e usabilidade para o público-alvo.

De forma geral, este trabalho demonstra que o investimento em tecnologia aplicada à saúde e ao cuidado de idosos não apenas facilita a rotina de usuários e cuidadores, mas também contribui para a promoção de autonomia, segurança e qualidade de vida. Os resultados obtidos evidenciam que, mesmo diante de limitações técnicas, é possível desenvolver sistemas relevantes e funcionais, que podem servir de referência para projetos futuros. Assim, o \nomeprojeto não apenas cumpre sua função prática, mas também reforça a importância de soluções inovadoras e adaptáveis na interface entre tecnologia e cuidado humano.
