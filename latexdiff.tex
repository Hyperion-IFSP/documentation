...
%DIF DEL antigo.tex   Mon Nov 24 17:31:56 2025
%DIF ADD atual.tex    Mon Nov 24 17:12:11 2025
...
\begin{abstract}
    This document \DIFdelbegin \DIFdel{describes the current architecture of the software designated as ``}%DIFDELCMD < \nomeprojeto%%%
\DIFdel{``, which aims to help people maintain their medication supply; and for caregivers maintain medication supplies for all their dependents. In addition, it will offer a scheduling service so that the user can track appointments and blood glucose and blood pressure levels. The content of this document discusses its concept and application, in addition to the data modeling and tools used}\DIFdelend \DIFaddbegin \DIFadd{presents "MyMed", an application designed to assist caregivers in managing the health routines of elderly dependents, aiming to centralize medication control, appointment scheduling, and vital sign monitoring. The methodology included qualitative and quantitative research with caregivers to define requirements, followed by development using the Scrum framework, Java Spring Boot for the back-end, and Angular for the front-end. Results from internal and external tests validated the core functionalities, including CRUD operations, external API integrations for medication search, and health tracking graphs, demonstrating the system's usability despite some identified enhancement opportunities and performance limitations}\DIFaddend .
\end{abstract}

\DIFdelbegin %DIFDELCMD < \begin{resumo1} 
%DIFDELCMD <   %%%
\DIFdel{Este documento descreve a arquitetura presente do software designado como ``}%DIFDELCMD < \nomeprojeto%%%
\DIFdel{``, ele visa auxiliar pessoas a manter seu estoque de medicamentos, e para cuidadores manter estoque de medicamentos de todos seus dependentes. Além disso, oferecerá um serviço de agenda para que o usuário acompanhe consultas e taxas de glicemia e pressão. O conteúdo deste documento discorre sobre seu conceito e aplicação, além da modelagem de dados e ferramentas utilizadas.
}%DIFDELCMD < \end{resumo1}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{resumo} 
    \DIFadd{Este documento apresenta o "MyMed", uma aplicação projetada para auxiliar cuidadores na gestão das rotinas de saúde de idosos dependentes, visando centralizar o controle de medicamentos, agendamento de consultas e monitoramento de sinais vitais. A metodologia utilizada incluiu pesquisas qualitativas e quantitativas com cuidadores para definição de requisitos, seguidas pelo desenvolvimento utilizando o framework Scrum, Java Spring Boot no back-end e Angular no front-end. Os resultados dos testes internos e externos validaram as funcionalidades principais, incluindo operações de CRUD, integrações com APIs externas para busca de medicamentos e gráficos de acompanhamento de saúde, demonstrando a usabilidade do sistema apesar de algumas limitações de desempenho e oportunidades para melhorias identificadas.
}\end{resumo}
\DIFaddend 
...
Visando superar essas limitações, o projeto propõe o desenvolvimento de uma plataforma focada exclusivamente no cuidador de idosos, com funcionalidades que centralizam o monitoramento e a gestão de \DIFdelbegin \DIFdel{tratramentos}\DIFdelend \DIFaddbegin \DIFadd{tratamnentos}\DIFaddend . A ferramenta inclui lembretes para administração dos remédios, agendamentos de consultadas com calendário integrado, indicação de locais para compra, pesquisa de preços e criação de relatórios médicos - tudo focado na interface intuitiva e direta. Assim, busca-se reduzir a sobrecarga dos cuidadores, proporcionando mais autonamia e eficiência no desempenho de suas funções.
\DIFaddbegin 

\DIFaddend \subsection{Objetivos}

\DIFdelbegin \DIFdel{Para o desenvolvimento do sistema, será adotado como referência uma diretriz de desenvolvimento que se baseará na comunicação constante com o usuário e controle das }\DIFdelend \DIFaddbegin \DIFadd{O MyMed tem por objetivo oferecer uma plataforma que facilite o gerenciamento de tratamento de idosos, reunindo todas }\DIFaddend informações \DIFaddbegin \DIFadd{necessárias e funções úteis para o acompanhamento de pacientes}\DIFaddend .

\subsubsection{Objetivo Geral}

\DIFdelbegin \DIFdel{Desenvolver uma plataforma que auxilie }\DIFdelend \DIFaddbegin \DIFadd{O MyMed visa auxiliar }\DIFaddend os cuidadores de idosos a manter de forma simples, organizada e eficaz o acompanhamento de seus pacientes. O sistema irá permitir \DIFaddbegin \DIFadd{o }\DIFaddend registro de medicamentos utilizados, consultas agendadas e realizadas \DIFdelbegin \DIFdel{, com }\DIFdelend \DIFaddbegin \DIFadd{e a }\DIFaddend possibilidade de gerar relatórios e gráficos personalizados de acordo com os tratamentos em andamento \DIFdelbegin \DIFdel{(como controle de glicemia e hipertensão).
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Além disso, o sistema poderá enviar alertas sobre o estoque de medicamentos de todos os pacientes vinculados a uma conta, indicando a estimativa de tempo de consumo restante e realizando pesquisas de preços para reposição. 
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Também contará com um módulo de registros de dados médicos, onde o cuidador poderá inserir informações essenciais, como índices de glicemia e pressão arterial, datas de retorno, novas prescrições }\DIFdelend e informações \DIFdelbegin \DIFdel{relevantes de um paciente}\DIFdelend \DIFaddbegin \DIFadd{necessárias}\DIFaddend .

\subsubsection{Objetivos Específicos}

\DIFdelbegin \DIFdel{A fim de }\DIFdelend \DIFaddbegin \DIFadd{Para }\DIFaddend alcançar o objetivo geral da proposta apresentada, foram delineados \DIFdelbegin \DIFdel{os seguintes }\DIFdelend objetivos específicos \DIFaddbegin \DIFadd{para esta meta}\DIFaddend :

\begin{enumerate}
    \item Conduzir entrevistas com cuidadores registrados e funcionários de casas de repouso \DIFdelbegin \DIFdel{. Analisar resultados para direcionar a }\DIFdelend \DIFaddbegin \DIFadd{e analisar os resultados para }\DIFaddend um desenvolvimento próximo ao usuário final da aplicação\DIFdelbegin \DIFdel{.
    }\DIFdelend \DIFaddbegin \DIFadd{;
    }\DIFaddend \item Realizar uma pesquisa de mercado de aplicações similares, \DIFdelbegin \DIFdel{a fim de criar }\DIFdelend \DIFaddbegin \DIFadd{para que haja a criação de }\DIFaddend uma solução única \DIFdelbegin \DIFdel{ao usuário voltada ao }\DIFdelend \DIFaddbegin \DIFadd{que possua o }\DIFaddend melhoramento de recursos já existentes\DIFdelbegin \DIFdel{e }\DIFdelend \DIFaddbegin \DIFadd{, a }\DIFaddend implantação de recomendações \DIFdelbegin \DIFdel{destes }\DIFdelend \DIFaddbegin \DIFadd{e novas funcionalidades úteis aos }\DIFaddend usuários\DIFdelbegin \DIFdel{. 
    }\DIFdelend \DIFaddbegin \DIFadd{;
    }\DIFaddend \item Desenvolver um sistema que, com uma interface simples e intuitiva, além de lembretes que auxilie o usuário a manter esses tratamentos, automatize tarefas banais e auxilie o controle por parte do cuidador\DIFdelbegin \DIFdel{.
    }\DIFdelend \DIFaddbegin \DIFadd{;
    }\item \DIFadd{Garantir o envio de alertas sobre o estoque de medicamentos de todos os pacientes vinculados a uma conta, a estimativa de tempo de consumo restante e possibilitar a pesquisa de preços para reposição;
    }\item \DIFadd{Possuir um módulo de registros de dados médicos, onde o cuidador poderá inserir informações essenciais, como índices de glicemia e pressão arterial, datas de retorno, novas prescrições e informações relevantes de um paciente;
    }\DIFaddend \item Aplicar as funcionalidades do sistema de forma empírica em um público-alvo a fim de aprimorar o sistema e torná-lo útil ao usuário final.
\end{enumerate}


\subsection{Justificativa}    

Segundo \DIFaddbegin \DIFadd{as autoras }\DIFaddend \citeonline{mariacristina2008}, o consumo de um medicamento ou o acompanhamento de consultas de rotina podem se tornar algo supérfluo na rotina de um paciente que os realiza com frequência, podendo acarretar em um esquecimento de tais compromissos. 

\DIFdelbegin \DIFdel{A partir disso}\DIFdelend \DIFaddbegin \DIFadd{Em paralelo}\DIFaddend , foi realizada uma pesquisa com o \DIFdelbegin \DIFdel{público geral no }\DIFdelend \DIFaddbegin \DIFadd{público-geral para definição de um escopo. No }\DIFaddend formato de um formulário online \DIFdelbegin \DIFdel{. Foi apontado que 70}\DIFdelend \DIFaddbegin \DIFadd{e com 45 respostas, aproximadamente 71}\DIFaddend \% dos participantes utilizam pouco ou muito pouco de serviços tecnológicos de saúde; dos que utilizam, 63\% relatam não corresponder às suas expectativas. Cerca de 62\% dos participantes \DIFdelbegin \DIFdel{têm }\DIFdelend \DIFaddbegin \DIFadd{relatam possuir }\DIFaddend grandes dificuldades em lembrar de datas de consultas, e 81\% relatam problemas no horário de consumo de remédios. \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{O }\textit{\DIFdel{feedback}} %DIFAUXCMD
\DIFdel{constante de usuários de sistemas existentes é uma das bases do desenvolvimento, e trará uma solução prática ao público-alvo que aperfeiçoe as aplicações já utilizadas.
}\DIFdelend \DIFaddbegin \DIFadd{O questionário aplicado nesta etapa se encontra no }\autoref{pesquisas}\DIFadd{.
}

\DIFaddend Desta forma, \DIFdelbegin \DIFdel{esse documento, demonstra }\DIFdelend \DIFaddbegin \DIFadd{após }\DIFaddend a \DIFdelbegin \DIFdel{necessidade de tal sistema, destinada a }\DIFdelend \DIFaddbegin \DIFadd{síntese destes dados, pode-se estabelecer uma lacuna que demonstra a utilidade deste sistema, priorizando acessibilidade a um sistema de qualidade e visando mitigar as necessidades dos }\DIFaddend usuários \DIFdelbegin \DIFdel{que necessitam ou desejam }\DIFdelend \DIFaddbegin \DIFadd{de }\DIFaddend um melhor gerenciamento dos tratamentos de seus pacientes \DIFdelbegin \DIFdel{, sejam eles medicamentosou consultas ; }\DIFdelend \DIFaddbegin \DIFadd{— sejam relacionados aos medicamentos, as consultas ou quaisquer outras informações pertinentes a estes prontuários — }\DIFaddend a fim de manter a saúde destes bem condicionadas e supervisionadas.
...
A telemedicina não é destinada à substituição do médico, mas sim como uma ferramenta assistiva que \DIFdelbegin \DIFdel{suavizará processos para ambos os grupos de }\DIFdelend \DIFaddbegin \DIFadd{otimiza processos para }\DIFaddend pacientes e profissionais. \DIFdelbegin \DIFdel{É, ainda, 
uma forma}\DIFdelend \DIFaddbegin \DIFadd{Desta forma, se apresenta também como um instrumento }\DIFaddend de democratização \DIFdelbegin \DIFdel{de serviços de }\DIFdelend \DIFaddbegin \DIFadd{da }\DIFaddend saúde\DIFdelbegin \DIFdel{, pois }\DIFdelend \DIFaddbegin \DIFadd{: enquanto }\DIFaddend muitas regiões \DIFdelbegin \DIFdel{não dispõem de tais serviços de forma prática \mbox{%DIFAUXCMD
\cite{Schaefer2023telemedicina}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{carecem de infraestrutura médica física, a tecnologia possui maior alcançabilidade para áreas remotas ou isoladas, conforme apontam \mbox{%DIFAUXCMD
\citeonline{Schaefer2023telemedicina}}\hskip0pt%DIFAUXCMD
}\DIFaddend .
...
Telemonitoramento, segundo \citeonline{petraroli2018biotelemetria}, é uma subárea da medicina, que permite o monitoramento e gerenciamento de dispositivos e sensores médicos via software para aumentar a eficiência dos processos. Em seu artigo, foi analisado a plataforma de monitoramento geriátrico de doenças crônicas \DIFdelbegin \DIFdel{`Virtual Monitor` }\DIFdelend \DIFaddbegin \textit{\DIFadd{Virtual Monitor}} \DIFaddend e seu potencial de investimento no mercado atual da telemedicina. A autora defende que sistemas como o de pesquisa possuem um atrativo comercial elevado no cenário atual somente se têm como foco a inovação de recursos de sustentabilidade e na competitividade. Diz, ainda, que para simplificar e escalar o acompanhamento de idosos, é de extrema necessidade uma solução que envolva cuidado centrado nas pessoas, viabilizando o mercado de cuidado suplementar.
...
\DIFdelbegin \DIFdel{São definidas }\DIFdelend \DIFaddbegin \DIFadd{O Ministério da Saúde e a Agência Nacional de Vigilância Sanitária já definiram }\DIFaddend normas que categorizam idosos não-autônomos em três termos de dependência: grau I, totalmente independentes; grau II, que necessitam de auxílio em até três atividades básicas diárias; grau III, que necessitam auxílio em todas as tarefas de autocuidado. É também posto um limite para cuidadores de idosos: em uma jornada de trabalho de oito horas diárias e cinco dias por semana, o profissional de uma instituição de cuidado pode auxiliar até 20 idosos com grau de dependência I, até 10 idosos com grau de dependência II ou 6 idosos com grau de dependência III \cite{ministeriosaude2021ilpi}.

\DIFdelbegin \DIFdel{Mesmo com a tentativa de limites, ainda há sobrecarga para estes profissionais. Segundo a }\DIFdelend \DIFaddbegin \DIFadd{Na }\DIFaddend pesquisa de \citeonline{nunes2018sabe}, que inclui um grupo de 359 cuidadores do município de São Paulo - SP, a maioria dos cuidadores era familiar, do sexo feminino, com média de idade de 53,9 anos\DIFdelbegin \DIFdel{. Em seguida, foram analisados fatores }\DIFdelend \DIFaddbegin \DIFadd{, percebe-se que mesmo com a imposição de limites, ainda existe uma sobrecarga para estes profissionais.
}

\DIFadd{Fatores }\DIFaddend como a disfunção familiar (incapacidade de uma família cobrir as necessidades básicas de um indivíduo como apoio emocional e financeiro) e o excesso de trabalho durante longas horas \DIFdelbegin \DIFdel{contribuem para }\DIFdelend \DIFaddbegin \DIFadd{se destacaram por contribuir com }\DIFaddend o estresse do profissional de cuidado, que somam mais de um terço do grupo de pesquisa. \DIFdelbegin \DIFdel{São utilizados de exemplos os }\DIFdelend \DIFaddbegin \DIFadd{Os }\DIFaddend cuidadores familiares, \DIFdelbegin \DIFdel{que }\DIFdelend mesmo tendo uma grande intimidade com o dependente, ainda sofrem com situações que exigem uma parcela próxima ao total de seu tempo. 
\DIFdelbegin \DIFdel{A autora finaliza com um apelo a instituições públicas, que não fornecem recursos suficientes para a manutenção pessoal de profissionais de cuidado, a fim de um exercício melhor de suas atividades.
}\DIFdelend 
...
\DIFaddbegin \DIFadd{Desta forma, surge um apelo principalmente às instituições públicas, pelo fornecimento de recursos suficientes para a manutenção pessoal de profissionais de cuidado e seus serviços, a fim de possibilitar o exercício adequado e apropriado de suas atividades.
}


\DIFaddend \section{Metodologia}
...
Os métodos de pesquisa \DIFdelbegin \DIFdel{do }\DIFdelend \DIFaddbegin \DIFadd{utilizados no }\DIFaddend projeto são divididas em duas partes: a pesquisa qualitativa, \DIFdelbegin \DIFdel{que visa entender }\DIFdelend \DIFaddbegin \DIFadd{destinada a compreender }\DIFaddend o público \DIFdelbegin \DIFdel{no }\DIFdelend geral e suas necessidades \DIFdelbegin \DIFdel{na aplicação; e a }\DIFdelend \DIFaddbegin \DIFadd{em relação ao uso de tecnologias para organização de cuidados com a saúde e, uma }\DIFaddend pesquisa quantitativa, \DIFdelbegin \DIFdel{planejada com o foco do sistema em }\DIFdelend \DIFaddbegin \DIFadd{direcionada especificamente ao nosso público-alvo, os }\DIFaddend cuidadores de idosos\DIFdelbegin \DIFdel{em casas de repouso, visando coletar informações de profissionais da área}\DIFdelend . 

\subsubsection{Instrumento de Pesquisa e Escalas Utilizadas}

O principal meio de pesquisa foi a realização de formulários online estruturados \DIFdelbegin \DIFdel{, desenvolvido com as }\DIFdelend \DIFaddbegin \DIFadd{desenvolvido com }\DIFaddend perguntas objetivas e subjetivas, visando identificar as principais demandas, dificuldades e práticas no cuidado com idosos.
...
\DIFaddbegin \subsubsubsection{Pesquisa com o Público Geral}
\DIFaddend Na primeira etapa, o formulário foi direcionado ao público geral com intuito de mapear as necessidades e desejos mais amplos. O questionário foi montado com 11 perguntas relacionadas ao gerenciamento de medicações e consultas pessoais e foram obtidas 45 respostas. 
\DIFdelbegin \DIFdel{Posteriormente na }\DIFdelend \DIFaddbegin 

\subsubsubsection{Pesquisa com o Público-alvo}
\DIFadd{Na }\DIFaddend segunda etapa, após um aprofundamento de escopo do projeto, um questionário com 10 perguntas relacionadas ao cuidado com idosos foi compartilhado com cuidadores \DIFdelbegin \DIFdel{profissionais, }\DIFdelend \DIFaddbegin \DIFadd{de idosos e }\DIFaddend instituições de longa permanência para idosos (casas de repouso), a fim de avaliarmos as necessidades do público alvo do \nomeprojeto. 

\DIFdelbegin \DIFdel{Todas as perguntas que foram feitas na primeira e segunda etapa }\DIFdelend \DIFaddbegin \DIFadd{As perguntas feitas em ambas pesquisas }\DIFaddend se encontram no \autoref{pesquisas}.
...
\DIFdelbegin %DIFDELCMD < \footnote[1]{%%%
\DIFdelend \DIFaddbegin \footnote{\DIFaddend Seção da terapêutica que se dedica ao estudo da dosagem certa dos medicamentos. Indicação da dosagem certa (adequada) de um medicamento tendo em conta os vários casos em que o mesmo pode ser prescrito.}, 
...
No \DIFdelbegin \DIFdel{front-end}\DIFdelend \DIFaddbegin \textit{\DIFadd{front-end}}\DIFaddend , optou-se pelo Angular 19, que permite o desenvolvimento de interfaces dinâmicas e reativas através de componentes. A biblioteca de componentes PrimeNG e a linguagem de design Material Design 3 foram incorporados para padronizar o design da aplicação. Para autenticação e autorização, foi utilizado o Auth0, que oferece uma solução segura e prática de Single Sign On (SSO).
...
Para o desenvolvimento do \DIFdelbegin \DIFdel{front-end}\DIFdelend \DIFaddbegin \textit{\DIFadd{front-end}}\DIFaddend , utilizou-se Angular 19 para a criação de páginas reativas, que mudam de estado. A biblioteca PrimeNG é usada para facilitar a criação de estilos dos elementos e simplificar a componentização e redução do código. Para a hospedagem, foi escolhida a plataforma Firebase.
...
%DIFDELCMD < %%%
\DIFdelend \begin{quadro}[!htbp]
    \caption{\label{quadro_integrantes}Integrantes da equipe}
        \begin{tabular}{|c|c|c|c|c|c|c|}
        \hline
            Papéis & Arthur & Fernando & Henriquy & Isabella & Lucas & Mateus \\ \hline
            Back-end & & & & & \checkmark & \DIFdelbegin %DIFDELCMD < \checkmark %%%
\DIFdelend \\ \hline
            Front-end & \checkmark & \checkmark & & \DIFdelbegin %DIFDELCMD < \checkmark %%%
\DIFdelend & & \\ \hline
...
A modelagem do sistema é uma etapa fundamental que visa traduzir os requisitos e regras de negócio em uma representação estruturada do software. Este processo envolve os casos de uso, a definição da arquitetura \DIFdelbegin \DIFdel{, }\DIFdelend e a estrutura de dados da aplicação. Para o \nomeprojeto, a modelagem foi detalhada por meio de diagramas \DIFdelbegin \DIFdel{s }\DIFdelend \DIFaddbegin \DIFadd{de casos }\DIFaddend de uso, que descrevem as interações entre os usuários e o sistema, o diagrama de arquitetura de sistema, que descreve como o sistema está \DIFdelbegin \DIFdel{estuturado }\DIFdelend \DIFaddbegin \DIFadd{estruturado }\DIFaddend tecnicamente e pelo modelo de entidade e relacionamento, que detalha a estrutura do banco de dados.
...
A comunicação entre esses contêineres ocorre predominantemente por meio de requisições HTTP. \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{A }\autoref{diagrama_software} \DIFadd{apresenta o diagrama de Arquitetura de Software completo, detalhando os contêineres, suas tecnologias e interações.
    ...
    \DIFdelbeginFL %DIFDELCMD < \label{fig:diagrama_software}
%DIFDELCMD <     %%%
\DIFdelendFL \DIFaddbeginFL \label{diagrama_software}
    \DIFaddendFL \fonte{Autores.}
\end{figure}
...
Para garantir a segurança na comunicação entre os usuários e a aplicação, foi configurado o protocolo HTTPS com suporte a TLS, utilizando certificados digitais válidos e atualizados. A certificação foi realizada através da integração com o serviço Let's Encrypt através do Render para o back-end e Gerenciado pelo Google para o \DIFdelbegin \DIFdel{front-end }\DIFdelend \DIFaddbegin \textit{\DIFadd{front-end}} \DIFaddend através do Firebase Hosting, assegurando criptografia ponta a ponta durante as transações de dados. A configuração foi validada na ferramenta SSL Labs, onde o ambiente obteve a nota máxima A+.
\DIFaddbegin \DIFadd{A }\autoref{testes-criptografia} \DIFadd{apresenta, respectivamente o resultado dos testes de criptografia realizados no }\textit{\DIFadd{front-end}} \DIFadd{e no }\textit{\DIFadd{back-end}} \DIFadd{da aplicação, confirmando a eficácia das medidas de segurança implementadas.
}\DIFaddend 
...
\begin{figure}[h!]
    ...
    \end{subfigure}
    \DIFdelbeginFL %DIFDELCMD < \label{fig:testes-criptografia}
%DIFDELCMD <     %%%
\DIFdelendFL \caption{Resultados dos testes de criptografia.}
    \DIFaddbeginFL \label{testes-criptografia}
    \DIFaddendFL \fonte{Autores.}
\end{figure}
...
Esta seção apresenta os principais resultados obtidos ao longo do desenvolvimento do projeto, bem como a análise crítica desses achados. São expostos os avanços alcançados, as limitações identificadas e os impactos observados no desempenho e na usabilidade do sistema. Além disso, \DIFdelbegin \DIFdel{busca-se discutir }\DIFdelend \DIFaddbegin \DIFadd{discute-se }\DIFaddend de que forma os resultados se relacionam com os objetivos propostos inicialmente, destacando pontos fortes, fragilidades e oportunidades de melhoria para trabalhos futuros. \DIFaddbegin \DIFadd{Também são examinados aspectos do funcionamento da plataforma, considerando tanto o }\textit{\DIFadd{frontend}} \DIFadd{quanto o }\textit{\DIFadd{backend}}\DIFadd{, a partir de testes e validações realizadas durante o processo de desenvolvimento.
}\DIFaddend 
...
Os resultados \DIFdelbegin \DIFdel{positivos demonstram a correta }\DIFdelend \DIFaddbegin \DIFadd{obtidos nos testes internos demonstram a }\DIFaddend execução \DIFaddbegin \DIFadd{correta }\DIFaddend das funcionalidades primárias do sistema. \DIFdelbegin \DIFdel{Em um cenário de sucesso, todas }\DIFdelend \DIFaddbegin \DIFadd{Todas }\DIFaddend as operações de CRUD (Criação, Edição e Exclusão) para usuários, administradores, pacientes, consultas e informações adicionais de perfil \DIFdelbegin \DIFdel{são persistidas com sucesso }\DIFdelend \DIFaddbegin \DIFadd{foram persistidas adequadamente }\DIFaddend no banco de dados, \DIFdelbegin \DIFdel{confirmando }\DIFdelend \DIFaddbegin \DIFadd{comprovando }\DIFaddend a eficácia da lógica de negócios e \DIFdelbegin \DIFdel{da query }\DIFdelend \DIFaddbegin \DIFadd{das consultas }\DIFaddend SQL. Além disso, a adição de medicamentos a um tratamento e a manutenção dos registros cardiovasculares e glicêmicos foram concluídas \DIFaddbegin \DIFadd{com êxito}\DIFaddend , indicando que os dados brutos de saúde \DIFdelbegin \DIFdel{são }\DIFdelend \DIFaddbegin \DIFadd{estão sendo }\DIFaddend armazenados corretamente.

A integração com serviços externos também apresentou \DIFdelbegin \DIFdel{sucesso}\DIFdelend \DIFaddbegin \DIFadd{funcionamento adequado}\DIFaddend : o sistema foi capaz de pesquisar e visualizar farmácias e medicamentos \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{à }\DIFaddend venda ao se conectar \DIFdelbegin \DIFdel{com sucesso }\DIFdelend às APIs externas, confirmando \DIFdelbegin \DIFdel{a funcionalidade adequada }\DIFdelend \DIFaddbegin \DIFadd{o bom desempenho }\DIFaddend dos mecanismos de comunicação e autenticação. Por fim, a visualização correta de consultas por filtro de data e a exibição dos gráficos de adesão e intercorrências \DIFdelbegin \DIFdel{indicam que a }\DIFdelend \DIFaddbegin \DIFadd{demonstram que o processo de }\DIFaddend recuperação e apresentação de dados \DIFdelbegin \DIFdel{funcionam conforme o esperado.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{está ocorrendo conforme o previsto.
}\DIFaddend \subsubsection{Aspectos negativos}
...
Os resultados positivos confirmam a solidez do \DIFdelbegin \DIFdel{back-end }\DIFdelend \DIFaddbegin \textit{\DIFadd{back-end}} \DIFaddend do sistema em suas operações fundamentais. Em condições ideais de rede e conectividade, as operações de CRUD relacionadas aos usuários, pacientes e consultas foram concluídas com \DIFdelbegin \DIFdel{sucesso}\DIFdelend \DIFaddbegin \DIFadd{êxito}\DIFaddend . Isso valida a lógica de negócios e a integridade das transações sob uma variedade de dispositivos e outros ambientes. A capacidade de adicionar medicamentos a um tratamento e registro de dados contínuos de saúde, como pressão arterial e nível de glicose, também deram certo. A integração com as APIs externas demonstrou ser robusta, \DIFdelbegin \DIFdel{conseguindo permitir }\DIFdelend \DIFaddbegin \DIFadd{permitindo }\DIFaddend a pesquisa de \DIFdelbegin \DIFdel{farmáciase }\DIFdelend \DIFaddbegin \DIFadd{farmácias e }\DIFaddend a visualização de medicamentos de forma rápida e precisa para a maioria dos testadores.

\subsubsection{Aspectos negativos}
Alguns problemas foram identificados no processo de testes externos foram que o login não é validado corretamente no \DIFdelbegin \DIFdel{backend}\DIFdelend \DIFaddbegin \textit{\DIFadd{backend}}\DIFaddend , comprometendo a segurança do sistema. Além disso, não foi possível abrir a foto do paciente no \DIFdelbegin \DIFdel{backend}\DIFdelend \DIFaddbegin \textit{\DIFadd{backend}}\DIFaddend , o que limita a visualização de informações relevantes. Outro fator crítico está relacionado ao tempo de inicialização da API, que pode levar alguns segundos até ficar ativa, impedindo o acesso imediato à aplicação. Soma-se a isso a ausência de implementação das claims/roles, permitindo que em alguns cenários, usuários realizem requisições sem restrições, o que representa um risco de uso indevido.
...
\input{sections/pesquisas}

\newpage

\chapter{Requisitos Funcionais\label{requisitos_f}}

\input{sections/requisitos_f}
